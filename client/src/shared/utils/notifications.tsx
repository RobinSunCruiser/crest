 ...
 * });
 *
 * // Warning notification with custom ID for later reference
 * showNotification({
 *   id: 'connection-warning',
 *   title: 'Connection Issue',
 *   message: 'Using cached data due to weak connection.',
 *   type: 'warning',
 *   autoClose: 5000
 * });
 * ```
 */
export const showNotification = ({ title, message, type, id, autoClose = false }: NotificationOptions) => {
    const iconColor = type === 'success' ? 'green' : type === 'error' ? 'red' : type === 'warning' ? 'orange' : 'blue';
    const iconType = type === 'success' ? <FaCheck /> : type === 'error' ? <MdClose size={remToPx(1.5)} /> : type === 'warning' ? <FaInfo size={remToPx(1)} /> : <FaInfo />;
