...
export const forwardToLLMAdapter = async (
  ...
  try {
    const modelInfos: IModelInfo[] = await llmAdapterManager.getAvailableModels();
    ...
    const onChunk = (message.data.modelOptions && message.data.modelOptions.stream) 
      ? (chunk: string) => {
          connection.emit({
    ...
    if (
      ...
      try {
        ...
        log.error(`Error reinitializing adapters: ${String(reinitError)}`, "llmHandler");
      }
...
export const listLLMs = async (
  ...
  try {
    // Retrieve models from all adapters
