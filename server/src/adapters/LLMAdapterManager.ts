...
import { IAdapterConfig, IModelInfo } from "../interfaces";
...
export class LLMAdapterManager {
      ...
      for (const adapterID in adapters) {
        ...
        try {
          ...
          if (await adapter.isAvailable()) {
            ...
            log.debug(`${provider} adapter '${adapterID}' unavailable - check connection to ${baseUrl} | API key validity | service status`, "LLMAdapterManager");
  ...
  async getAvailableModels(): Promise<IModelInfo[]> {
    ...
    for (const [adapterID, adapter] of this.adapterPool.entries()) {
      ...
      filteredModels.forEach((modelName: string) => {
        models.push({
